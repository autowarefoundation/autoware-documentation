...
The Control Component consists of the two modules. The `trajectory_follower` generates a vehicle control command necessary to follow a reference trajectory generated by the planning module. The command includes, for example, the desired steering angle and target speed. The `vehicle_command_gate` is responsible for switching between control commands generated by sources other than the trajectory follower, such as MRM module, and filtering control values to prevent abnormal values.
