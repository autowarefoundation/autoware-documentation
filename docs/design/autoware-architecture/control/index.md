...
This document presents the design concept of the Control Comppnent. The content is as follows:
...
The Control Component generates the vehicle interface messages to which the Vehicle Component subscribes. The generated control messages are computed based on the reference trajectories from the Planning Component.

![control-component](image/control-component.drawio.svg)

The trajectory follower generates a vehicle control command necessary to follow a reference trajectory generated by the planning module. The command includes, for example, the desired steering angle and target speed. In addition, the vehicle command gate is responsible for switching between control commands generated by sources other than the trajectory follower, such as MRM module, and filtering control values to prevent abnormal values.
